...
function addImageSourcesFunctions(scene) {
    ...
    scene.extractPaths = function() {
        ...
        if (checkIntersect == null){ // no intersections found, add the path
            scene.paths.push([leftEarNode, scene.source]);
        }
            ...
            if (vec3.distance(p0, checkIntersect.PMin ) > vec3.distance(p0, scene.source.pos)){
                scene.paths.push([leftEarNode, scene.source]); //direct path not blocked-- add it to the path array
            }
        }
        ...
        for(var s=1; s<scene.imsources.length; s++){ // check path from receiver to every image source (not including direct path)
            ...
            var arrayVert = [];
            arrayVert.push(leftEarNode);
            scenePathFinder(scene, p0, image, arrayVert, null);
        }

        scene.pathsBreakPoint = scene.paths.length;

        var rightEar = vec3.create();
        vec3.scaleAndAdd(rightEar, scene.receiver.pos, scene.receiver.right, earWidth);
        var rightEarNode = {pos:rightEar, rcoeff:1.0};
        var p0 = rightEar;
        var v = vec3.create();
        vec3.subtract(v, scene.source.pos, p0); // ray from receiver to source --> source - receiver
        var normV = vec3.create();
        ...
        checkIntersect = scene.rayIntersectFaces(p0, normV, scene, mat4.create(), null); //check for occlusions
        if (checkIntersect == null){ // no intersections found, add the path
            scene.paths.push([rightEarNode, scene.source]);
        }
        else{ // the path is blocked by a plane
            //check if intersection point is BEHIND the source using distance between IP and Receiver & Source and Receiver
            if (vec3.distance(p0, checkIntersect.PMin ) > vec3.distance(p0, scene.source.pos)){
                scene.paths.push([rightEarNode, scene.source]); //direct path not blocked-- add it to the path array
            }
        }
    ...
    scene.computeImpulseResponse = function(Fs) {
        ...
        var sampleIndicesL = [];
        var magnitudesL = [];
        for (var i = 0; i < scene.pathsBreakPoint; i++) {
            ...
            sampleIndicesL.push(sampleIndex);
            magnitudesL.push(magnitude);
        }

        var N = 0;
        for (var i = 0; i < sampleIndicesL.length; i++) {
            if (sampleIndicesL[i] > N) {
                N = sampleIndicesL[i];
            }
        }
        scene.impulseRespL = new Float32Array(N+1);
        for (var i = 0; i < sampleIndicesL.length; i++) {
            scene.impulseRespL[sampleIndicesL[i]] += magnitudesL[i];
        }

        if (scene.paths.length == scene.pathsBreakPoint) {
            scene.impulseRespR = [];
            return;
        }
        var sampleIndicesR = [];
        var magnitudesR = [];
        for (var i = scene.pathsBreakPoint; i < scene.paths.length; i++) {
            var path = scene.paths[i];
            var length = 0;
            var magnitude = 1.0;
            for (var j = 0; j < path.length-1; j++) {
            ...
            var sampleIndex = Math.round(length / SVel * Fs);
            sampleIndicesR.push(sampleIndex);
            magnitudesR.push(magnitude);
        }

        var N = 0;
        for (var i = 0; i < sampleIndicesR.length; i++) {
            if (sampleIndicesR[i] > N) {
                N = sampleIndicesR[i];
            }
        }
        scene.impulseRespR = new Float32Array(N+1);
        for (var i = 0; i < sampleIndicesR.length; i++) {
            scene.impulseRespR[sampleIndicesR[i]] += magnitudesR[i];
        }
